...
function hasDetails(garment: Garment) {
  return Boolean(
    garment.fabricType ||
    garment.fabricQuantity ||
    garment.quantity !== '1' ||
    garment.measurementsNote ||
    garment.fabricPhoto ||
    garment.modelPhoto ||
    garment.fabricLinks?.length ||
    garment.modelLinks?.length ||
    garment.measurements?.some((measurement) => measurement.value),
  );
}

export function GarmentsEditor({ garments, baseMeasurements, onChange }: Props) {
  const [openIds, setOpenIds] = useState(() => new Set(garments.filter(hasDetails).map((garment) => garment.id)));
  ...
  return (
    <div className="flex flex-col gap-4">
      {garments.map((g, index) => {
        const detailsOpen = openIds.has(g.id);
        const Icon = detailsOpen ? ChevronDown : ChevronRight;
        ...
        return (
          <section key={g.id} className="flex flex-col gap-4 rounded-lg border border-border/70 bg-background/50 p-4">
            ...
            <button
              type="button"
              onClick={() => toggle(g.id)}
              className="flex items-center justify-between gap-3 rounded-lg border border-border/70 px-3 py-2 text-left text-sm font-medium text-muted-foreground transition-colors hover:text-foreground"
            >
              Détails du vêtement
